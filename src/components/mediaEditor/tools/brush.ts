export class PrimitiveBrush {
  ...
  private _draw() {
    ...
    for(let i = 0; i < 5; i++) {
      const offsetX = [-4, -2, 0, 2, 4][i]; // Adjust offsets as needed
      const offsetY = [-4, -2, 0, 2, 4][i]; // Adjust offsets as needed

      this.ctx.beginPath();
      this.ctx.moveTo(pt0.x + offsetX, pt0.y + offsetY);

      for(let j = startIndex; j < this.lastLength; j++) {
        const pt = this.workingStrokes[j];
        this.ctx.lineTo(pt.x + offsetX, pt.y + offsetY);
      }
